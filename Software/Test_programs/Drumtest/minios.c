...
static uint8_t tempo_ctr = 0;
static uint8_t int_ctr = 0;

extern uint8_t cur_tempo;
extern uint8_t reg0E;
...
/* =====================================================================
Interrupt handler. Handles four interrupts:
    - RTC from YM3806, 10 ms interval
    - serial tx from UART
    - serial rx from UART
    - timer int from YM2154
--------------------------------------------------------------------- */
...
void intser(void) __critical __interrupt(0)
{
    ...
    // If no interrupts found yet, it must be from RYP4
    if (!found_int)
    {
        reg0E = RYP4_int_ack;
